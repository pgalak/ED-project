...
export class PostCommentsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.data$ = forkJoin([this.postService.fetchPost(userId, postId),
    this.postService.fetchComments(userId, postId)]).pipe(
      map(([post, comments]: [UserPost, UserComment[]]) => new
      PostAndCommentsPage(post, comments))
    );
  }
}
